...
//  taskapp
//
//  Created by CapriCole on 2021/03/31.
//
...
class InputViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        contentsTextView.layer.borderColor = UIColor(red:0.9,green:0.9,blue:0.9,alpha:1.0).cgColor
    }
    ...
    func setNotification(task: Task) {
        ...
        //タイトルと内容を設定（中身がない場合はメッセージ無しで音だけで通知になるので「(xxなし)」を表示する）
        if task.title == "" {
        ...
        center.add(request) {
            ...
            //errorがnilならローカル通知の登録に成功したと表示します。
